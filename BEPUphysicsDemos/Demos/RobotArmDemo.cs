 ...
 StandardDemo
#endif
    {
            ...
            : base(game)
        {
            ...
            var armBase = new Cylinder(new Vector3(0, 2, 0), 2.5m, 1, 40);
            Space.Add(armBase);
            ...
            Entity lowerArm = new Box(armBase.Position + new Vector3(0, 2, 0), 1, 3, .5m, 10);
            Space.Add(lowerArm);
            ...
            Entity upperArm = new Cylinder(lowerArm.Position + new Vector3(0, 3, 0), 3, .25m, 10);
            Space.Add(upperArm);

            //Swivel hinges allow motion around two axes.  Imagine a tablet PC's monitor hinge.
            elbow = new SwivelHingeJoint(lowerArm, upperArm, lowerArm.Position + new Vector3(0, 1.5m, 0), Vector3.Forward);
            ...
            var lowerPosition = upperArm.Position + new Vector3(-.65m, 1.6m, 0);

            CollisionRules clawPart1ARules = new CollisionRules();
            var bodies = new List<CompoundChildData>()
            {
                new CompoundChildData() { Entry = new CompoundShapeEntry(new BoxShape(1, .25m, .25m), lowerPosition, 3), CollisionRules = clawPart1ARules },
                new CompoundChildData() { Entry = new CompoundShapeEntry(new ConeShape(1, .125m), lowerPosition + new Vector3(-.375m, .4m, 0), 3), Material = new Material(2,2,0) }
            };

            var claw = new CompoundBody(bodies, 6);
            Space.Add(claw);

            clawHingeA = new RevoluteJoint(upperArm, claw, upperArm.Position + new Vector3(0, 1.5m, 0), Vector3.Forward);
            ...
            lowerPosition = upperArm.Position + new Vector3(.65m, 1.6m, 0);

            CollisionRules clawPart1BRules = new CollisionRules();
            bodies = new List<CompoundChildData>()
            {
                new CompoundChildData() { Entry = new CompoundShapeEntry(new BoxShape(1, .25m, .25m), lowerPosition, 3), CollisionRules = clawPart1BRules },
                new CompoundChildData() { Entry = new CompoundShapeEntry(new ConeShape(1, .125m), lowerPosition + new Vector3(.375m, .4m, 0), 3), Material = new Material(2,2,0) }
            };
            claw = new CompoundBody(bodies, 6);
            Space.Add(claw);

            clawHingeB = new RevoluteJoint(upperArm, claw, upperArm.Position + new Vector3(0, 1.5m, 0), Vector3.Forward);
            ...
            for (Fix64 k = 0; k < MathHelper.Pi * 2; k += MathHelper.Pi / 6)
            {
                Space.Add(new Box(new Vector3(Fix64.Cos(k) * 5.5m, 2, Fix64.Sin(k) * 5.5m), 1, 1, 1, 10));
            }
        ...
        public override void Update(Fix64 dt)
        {
            ...
            if (Game.KeyboardInput.IsKeyDown(Keys.O))
                elbow.TwistMotor.Settings.Servo.Goal += 1 * dt;
            if (Game.KeyboardInput.IsKeyDown(Keys.P))
                elbow.TwistMotor.Settings.Servo.Goal -= 1 * dt;

            if (Game.KeyboardInput.IsKeyDown(Keys.OemOpenBrackets))
            {
                clawHingeA.Motor.Settings.Servo.Goal = MathHelper.Max(clawHingeA.Motor.Settings.Servo.Goal - 1.5m * dt, clawHingeA.Limit.MinimumAngle);
                clawHingeB.Motor.Settings.Servo.Goal = MathHelper.Min(clawHingeB.Motor.Settings.Servo.Goal + 1.5m * dt, clawHingeB.Limit.MaximumAngle);
            }
            if (Game.KeyboardInput.IsKeyDown(Keys.OemCloseBrackets))
            {
                clawHingeA.Motor.Settings.Servo.Goal = MathHelper.Min(clawHingeA.Motor.Settings.Servo.Goal + 1.5m * dt, clawHingeA.Limit.MaximumAngle);
                clawHingeB.Motor.Settings.Servo.Goal = MathHelper.Max(clawHingeB.Motor.Settings.Servo.Goal - 1.5m * dt, clawHingeB.Limit.MinimumAngle);
            }
